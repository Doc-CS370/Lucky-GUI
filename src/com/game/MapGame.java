...
public class MapGame extends JFrame{
	...
	InputStream stream = getClass().getResourceAsStream("/images/leftArrow.png");
	InputStream stream2 = getClass().getResourceAsStream("/images/rightArrow.png");

	ImageIcon larr = new ImageIcon(ImageIO.read(stream));
	ImageIcon rarr = new ImageIcon(ImageIO.read(stream2));
	...
	MapGame() throws IOException {
		...
		rightA.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (cardValue[5] < 9) {
					...
					cardValue[5] += 1;
					for(int i = 0;i<6;i++) {
						PaintPanel.setArray(i, PaintPanel.getP1Value(cardValue[i]) + ".png");
						try {
		...
		kill.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(GetCardLocation.currentCardNumber >= 44 && GetCardLocation.currentCardNumber <= 63 && 
					...
					else if(!witnessCheck.checkForWitness(map.Room,map.Player,countNum+1)) {
						chatRoom.append("There are witnesses.");
					}
					HighlightCards.setCheckCardsFalse();
					repaint();
				}
					...
					if(GetCardLocation.currentCardNumber >= 1 && GetCardLocation.currentCardNumber <= 43) {
						...
						if (countNum < PaintPanel.playerNum-1) {
							...
							if(playerAttack == countNum) {
								...
								PaintPanel.updateCardImg();
								HighlightCards.setCheckCardsFalse();
								GetCardLocation.currentCardNumber = 0;repaint();
								printAllPlayers();
								move.setEnabled(false);
								kill.setEnabled(false);
								draw.setEnabled(false);
		...
		MapGame.getChatRoom().setText("Welcome to Doctor Lucky!" + MapGame.getNextLine());
		MapGame.getChatRoom().append("Player 1, your turn." + MapGame.getNextLine());
		MapGame.getChatRoom().append("Player 1, you are in " + map.Room[PaintPanel.getPRoom(0)].getRoomFlavor() + MapGame.getNextLine());
