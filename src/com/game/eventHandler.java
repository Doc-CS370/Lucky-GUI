...
public class eventHandler {
    ...
    public static void drawCard(card[] Card, player[] player, int playerNum) {
        ...
        if (cnt == 0) {
            ...
            if(eligibleCards[randInt] >= 1 && eligibleCards[randInt]<=43) {
                MapGame.getChatRoom().append("You got a Failure Card. " + MapGame.getNextLine() +
                        Card[eligibleCards[randInt]].getCardFlavor() + MapGame.getNextLine());
            }else if(eligibleCards[randInt] >=44 && eligibleCards[randInt] <=63){
                MapGame.getChatRoom().append("You got a Weapon Card. " + MapGame.getNextLine() +
                        Card[eligibleCards[randInt]].getCardFlavor() + MapGame.getNextLine());

            }else if(eligibleCards[randInt]>=64 && eligibleCards[randInt]<=77){
                MapGame.getChatRoom().append("You got a Move Card. " + MapGame.getNextLine() +
                        Card[eligibleCards[randInt]].getCardFlavor() + MapGame.getNextLine());

            }else{
                MapGame.getChatRoom().append("You got a Room Card. " + MapGame.getNextLine() +
                        Card[eligibleCards[randInt]].getCardFlavor() + MapGame.getNextLine());
